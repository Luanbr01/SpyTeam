...
Sistema web para **gestão de assessoria esportiva, alunos, treinos, planejamento semanal e acompanhamento de desempenho**, desenvolvido com **Python, FastAPI, SQLAlchemy, PostgreSQL/SQLite, Jinja2, HTML, CSS e JavaScript**.
...
O SPY TEAM está preparado para rodar localmente com SQLite e em produção no **Railway com PostgreSQL**, mantendo um **Volume** separado para arquivos persistentes, como fotos de perfil, além de domínio próprio e envio de e-mails transacionais pelo **Resend**.
...
- [x] SQLite para desenvolvimento/compatibilidade
- [x] SQLAlchemy ORM
- [x] Deploy no Railway
...
- [x] Perfil do professor
- [x] E-mail verificado e troca de e-mail
- [x] Recuperação de senha por e-mail
- [x] Alteração de senha no painel
- [x] Foto de perfil
- [x] Edição de nome e usuário

## Aluno

- [x] Login
- [x] Primeiro acesso com cadastro obrigatório e verificação de e-mail
- [x] Troca de e-mail com confirmação
- [x] Foto de perfil
- [x] Edição de nome e usuário
- [x] Dashboard pessoal
- [x] Agenda semanal
...
| SQLAlchemy | ORM |
| PostgreSQL | Banco de dados recomendado em produção |
| SQLite | Desenvolvimento local e origem da migração |
| psycopg 3 | Driver PostgreSQL para SQLAlchemy |
...
| Railway | Hospedagem |
| Railway Volume | Persistência de arquivos enviados (ex.: fotos de perfil) |
...
flowchart TD
    ...
    ORM --> PostgreSQL[(PostgreSQL - produção)]
    ORM -. desenvolvimento local .-> SQLite[(SQLite)]

    FastAPI --> Resend[Resend API]
    Resend --> Email[E-mail do aluno]

    Railway[Railway] --> FastAPI
    Volume[Railway Volume /data] --> Uploads[Uploads persistentes / fotos]
```
...
O projeto utiliza **SQLAlchemy** com dois modos de banco:

- **PostgreSQL**: recomendado para produção no Railway;
- **SQLite**: mantido para desenvolvimento local, compatibilidade e migração de dados antigos.
...
1. `DATABASE_URL`, quando configurado — usado para PostgreSQL;
2. SQLite em `DATABASE_PATH`, quando não existe `DATABASE_URL`;
3. SQLite no Railway Volume, quando não existe `DATABASE_URL`;
4. `assessoria.db` na raiz do projeto para desenvolvimento local.
...
O driver utilizado para PostgreSQL é o **psycopg 3**. URLs Railway no formato `postgres://` ou `postgresql://` são normalizadas automaticamente pelo SPY TEAM para o dialeto `postgresql+psycopg://`.
...
## 8.9 Migração SQLite -> PostgreSQL
...
- lê o SQLite existente;
- cria as tabelas atuais no PostgreSQL;
- copia os registros preservando os IDs;
- mantém relacionamentos entre alunos, usuários, treinos, feedbacks, auditoria e notificações;
- ajusta as sequences do PostgreSQL após importar IDs explícitos;
- verifica a contagem final por tabela;
- **não apaga o SQLite original**.

Para uma migração segura no Railway, use temporariamente:

```env
POSTGRES_MIGRATION_URL=${{Postgres.DATABASE_URL}}
```

Execute a migração enquanto `DATABASE_URL` ainda não estiver apontando para o PostgreSQL. Depois da validação, configure:
...
Além das regras de aluno, exige que exista um e-mail confirmado/verificado para a conta.
...
## 10.1 Primeiro acesso e verificação de e-mail

Ao primeiro login de um aluno sem e-mail verificado:
...
O aluno informa e confirma o endereço. O sistema envia um link para:

```text
/verificar-email?token=...
```

O link expira em aproximadamente **30 minutos**. Enquanto o endereço não for confirmado, o aluno continua bloqueado fora da área normal e permanece no fluxo de primeiro acesso.

E-mails cadastrados antes da introdução dessa funcionalidade são marcados como verificados pela migração leve para preservar as contas existentes.

---

## 10.2 Validação, confirmação e troca de e-mail
...
- precisa ser confirmado duas vezes no formulário;
- precisa ser único entre as contas;
- só passa a valer depois do link de confirmação.

Aluno e professor podem solicitar troca de e-mail pelo perfil. Para trocar um endereço já existente, o usuário informa a senha atual. O e-mail anterior continua válido até o novo endereço ser confirmado.

Endpoint de troca:

```text
POST /api/me/email/troca
```

No primeiro acesso do aluno, o endpoint abaixo continua sendo usado para enviar a confirmação inicial:

```text
PATCH /api/me/email
```

Há um intervalo mínimo de aproximadamente 60 segundos entre solicitações de confirmação da mesma conta.
...
A recuperação funciona para **alunos e professor**, desde que a conta possua um e-mail verificado.
...
| GET | `/verificar-email` | Público com token | Confirmar cadastro/troca de e-mail |
| GET | `/health` | Público | Healthcheck |
| GET | `/home` | Professor | Dashboard |
| GET | `/professor/perfil` | Professor | Perfil, e-mail, foto e senha do professor |
...
| GET | `/aluno` | Aluno com e-mail verificado | Dashboard |
| GET | `/aluno/historico` | Aluno com e-mail verificado | Histórico |
| GET | `/aluno/perfil` | Aluno com e-mail verificado | Perfil |
...
| PATCH | `/api/me/email` | Aluno no primeiro acesso | Enviar confirmação do primeiro e-mail |
| POST | `/api/me/email/troca` | Logado | Solicitar troca/cadastro de e-mail com confirmação |
| GET | `/api/me/foto` | Logado | Obter foto de perfil |
| POST | `/api/me/foto` | Logado | Enviar foto PNG/JPG/WEBP de até 3 MB |
| DELETE | `/api/me/foto` | Logado | Remover foto de perfil |
| PATCH | `/api/me/senha` | Logado | Alterar senha pelo painel |
| POST | `/api/senha/esqueci` | Público | Solicitar recuperação para aluno ou professor |
...
Quando `DATABASE_URL` existe, ele tem prioridade e o SPY TEAM utiliza PostgreSQL.

### `POSTGRES_MIGRATION_URL`

Variável temporária usada somente durante a migração do SQLite atual para PostgreSQL:

```env
POSTGRES_MIGRATION_URL=${{Postgres.DATABASE_URL}}
```

Após a migração ser validada e `DATABASE_URL` ser ativado, essa variável pode ser removida.

### `DATABASE_PATH`

Mantido para SQLite local/legado. Exemplo:

```text
/data/assessoria.db
```

Quando `DATABASE_URL` está configurado, `DATABASE_PATH` não é utilizado como banco principal.
...
# Apenas durante a migração inicial
# POSTGRES_MIGRATION_URL=${{Postgres.DATABASE_URL}}
...
# 20. PostgreSQL e persistência no Railway

Em produção, o banco relacional recomendado do SPY TEAM é **PostgreSQL**. O Railway fornece ao serviço PostgreSQL a variável `DATABASE_URL`, que deve ser referenciada pelo serviço da aplicação.

O **Railway Volume continua conectado** ao SPY TEAM, mas deixa de ser o banco principal. Ele permanece responsável pelos arquivos enviados, como fotos de perfil.
...
SQLite legado preservado durante a transição:

```text
/data/assessoria.db
```

Não remova o SQLite imediatamente após a migração. Mantenha-o como backup até validar login, alunos, treinos, feedbacks, notificações e recuperação de senha no PostgreSQL.

O `RAILWAY_VOLUME_MOUNT_PATH` continua sendo usado para arquivos persistentes.
...
- aluno cadastra no primeiro acesso e precisa confirmar o endereço;
- professor pode cadastrar o próprio e-mail no perfil;
- e-mail precisa ser único;
- troca de e-mail exige confirmação do novo endereço;
- o endereço antigo permanece ativo até a confirmação do novo;
- recuperação é destinada a alunos e professor com e-mail verificado;
...
- banco recomendado em produção: PostgreSQL via `DATABASE_URL`;
- banco local/legado: `assessoria.db`;
- SQLite antigo pode permanecer em `/data/assessoria.db` como backup após a migração;
- o Railway Volume continua sendo necessário para fotos/arquivos;
...
- [x] suporte a PostgreSQL em produção;
...
- [x] troca de e-mail;
- [x] confirmação/verificação do e-mail;
- [x] recuperação de senha do professor;
- [x] alteração de senha do professor no painel;
- [x] foto de perfil para aluno e professor;
- [x] edição de dados pessoais (nome e usuário).

As fotos de perfil são armazenadas fora da pasta pública. Em produção, como o banco está em `/data/assessoria.db`, os arquivos ficam em:

```text
/data/uploads/perfis/
```

Dessa forma, as fotos também permanecem após novos deploys no Railway. A API aceita somente PNG, JPG ou WEBP e limita cada arquivo a 3 MB.
...
- [ ] editar nome/usuário do professor
- [ ] cadastrar e confirmar e-mail do professor
- [ ] trocar e-mail do professor
- [ ] alterar senha do professor pelo painel
- [ ] enviar/remover foto do professor

### Aluno

- [ ] primeiro e-mail + confirmação por link
- [ ] trocar e-mail
- [ ] editar nome/usuário
- [ ] enviar/remover foto
- [ ] dashboard
...
- [ ] token expira
- [ ] senha nova funciona
- [ ] recuperação funciona para aluno
- [ ] recuperação funciona para professor com e-mail verificado
...
A arquitetura atual foi mantida simples para facilitar manutenção e evolução durante o desenvolvimento, sem impedir futuras evoluções para componentes mais robustos, como Alembic, serviços de fila, observabilidade e uma suíte completa de testes automatizados.
